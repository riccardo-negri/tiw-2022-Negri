{ // avoid variables ending up in the global scope
    ...
    function HomeView() {
        ...
        this.editor = function (accounts) {
            ...
            // set breadcrumb name
            document.getElementById("breadcrumb-page").textContent = "Home"
            ...
            // event listeners for every VIEW link in the accounts list
    ...
    function AccountView() {
                ...
                if (x.readyState === XMLHttpRequest.DONE) {
                    ...
                    switch (x.status) {
                        case 200:
                            ...
                            self.editor(account, contacts, lastMonthTransactions, lastYearTransactions, previousTransactions)
                            break;
                        case 400: // bad request
                            displayGenericModal("Bad request", message)
                            break;
        ...
        this.editor = function (account, contacts, lastMonthTransactions, lastYearTransactions, previousTransactions) {
            // set breadcrumb name
            document.getElementById("breadcrumb-page").textContent = "Account"
            ...
            // add autocompletion
            let usernamesInContacts = []
            let usernamesAccountsInContacts = {}
            contacts.forEach(contact => {
                usernamesInContacts.push(contact.user.username)
                let temp = []
                contact.accountList.forEach(account => {
                    temp.push(accountCodeFormatter(account))
                })
                usernamesAccountsInContacts[contact.user.username.trim()] = temp
            })
            // set auto completer for beneficiary username input filed
    ...
    function TransactionDetailsView() {
        this.show = function (transactionID) {
            const self = this
            makeCall("GET", "get-transaction-details?id=" + transactionID, null, function (x) {
                if (x.readyState === XMLHttpRequest.DONE) {
                    const message = x.responseText;
                    switch (x.status) {
                        case 200:
                            ...
                            self.editor(transaction, isAccountInContacts, accountOrigin, accountDestination)
                            break;
                        case 400: // bad request
                            displayGenericModal("Bad request", message)
                            break;
        ...
        this.editor = function (transaction, isAccountInContacts, accountOrigin, accountDestination) {
            // display modal with transaction info
            ...
            if (!isAccountInContacts) {
                ...
                // listener associated with the added button
    ...
    function ProfileView() {
        ...
        this.editor = function (user) {
            ...
            // display user info
            const MODAL = `<div id="modal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="modal-default" aria-hidden="true"><div class="modal-dialog modal- modal-dialog-centered modal-" role="document"><div class="modal-content"><div class="modal-header"><h6 class="modal-title" id="modal-title-default">Your account details</h6></div><div class="modal-body"><div class="pb-2"><div class="d-flex flex-column"><span class="mb-2 text-xs">Username: <span class="text-dark font-weight-bold ms-sm-2">${user.username}</span></span><span class="mb-2 text-xs">Name: <span class="text-dark ms-sm-2 font-weight-bold">${user.name}</span></span><span class="mb-2 text-xs">Surname: <span class="text-dark ms-sm-2 font-weight-bold">${user.surname}</span></span><span class="mb-2 text-xs">Email: <span class="text-dark ms-sm-2 font-weight-bold">${user.email}</span></span></div></div></div><div id="modal-footer" class="modal-footer"><button id="close-modal-button" type="button" class="btn mb-0 bg-gradient-primary">Close</button></div></div></div></div>`
            setAndDisplayModal(MODAL)

            // close button
            document.getElementById("close-modal-button").addEventListener("click", () => {
                document.getElementById("profile-link").classList.remove("active")
                document.getElementById("account-link").style.display === "none" ? document.getElementById("home-link").classList.add("active") : document.getElementById("account-link").classList.add("active");
                document.getElementById("modal-container").innerHTML = ""
            })
        }
    }
