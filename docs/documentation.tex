...
\\
CONTO(\underline{codice}, saldo, utente)
\\
TRASFERIMENTO(\underline{ID}, data, importo, origine, destinazione)
...
CREATE TABLE `tiw_db`.`conto` (
	...
	CONSTRAINT `utente_conto` FOREIGN KEY (`utente`) 
		REFERENCES `tiw_db`.`utente` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
);
\end{minted}
...
CREATE TABLE `tiw_db`.`trasferimento` (
	...
	CONSTRAINT `trasferimento_origine` FOREIGN KEY (`origine`) 
		REFERENCES `tiw_db`.`conto` (`codice`)
